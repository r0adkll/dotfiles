...
scriptPath=$(echo "${0%/*}")
configPath=~/.config/zfs-health-check

# Set other paths.
limitCheckScrip=${scriptPath}/discord-webhook-data-limit-check.sh
lastMessageFile=~/.config/zfs-health-check/discord-webhook-last-message.json
...
sendWebhook() {
    ...
    if [ -z "${discordAttachmentPath}" ] || [ "${includeAttachment}" = "false" ]; then
        ...
        reponse=$(curl -s -H "Content-Type: application/json" -d "$discordJsonData" ${discordWebhookUrl})

    else

        # Send message with attachment
        response=$(curl -s -F payload_json="${discordJsonData}" -F "file1=@${discordAttachmentPath}" ${discordWebhookUrl})
...
discordWebhookUrl=$(< $DISCORD_ZFS_WEBHOOK)
discordRoleID=${DISCORD_ADMIN_ROLE_ID}
...
discordJson='{ "content":"'"${discordMessageContent}"'",
               "allowed_mentions": {
                 "roles": [ "'"${discordRoleID}"'" ]
               },
